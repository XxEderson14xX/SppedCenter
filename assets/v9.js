...
// Base: v9.js completo recuperado. Conserva Dashboard, descuentos,
// adicionales, OT, carga, ingresos y bitácora; actualiza Herramienta especial
// para integrarse con el DOM V11.1.
// ============================================================================
(() => {
...
async function dashboard(){
  ...
  if($('dash-avance-ordenes'))$('dash-avance-ordenes').innerHTML=(data.ordenes||[]).map(o=>`<div><strong>${o.folio}</strong> · ${o.placa} · ${o.realizados}/${o.total}<div class="v9-progreso"><span style="width:${o.total?o.realizados/o.total*100:0}%"></span></div></div>`).join('')||'<small>Sin órdenes abiertas.</small>';
  ...
  if($('dash-pagos'))$('dash-pagos').innerHTML=(data.ultimos_pagos||[]).map(p=>`<div>${p.folio} · $${money(p.importe)} · ${p.metodo}</div>`).join('')||'<small>Sin pagos hoy.</small>';
}
...
async function abrirOT(id){
  ...
  $('v9-ot-resumen').innerHTML=`<div class="v9-ot-resumen-grid"><div><small>Cotizacion</small><strong>${o.cotizaciones?.folio||'—'}</strong></div><div><small>Vehiculo</small><strong>${o.vehiculos?.placa||'—'} · ${o.vehiculos?.marca||''} ${o.vehiculos?.modelo||''}</strong></div><div><small>Estado</small><strong id="v9-ot-estado-visual" class="v9-ot-estado">${o.estado==='terminada'?'TERMINADA':'ABIERTA'}</strong></div></div>`;
...
$('v9-imprimir-ot')?.addEventListener('click',()=>{const a=obtenerAvanceOT(),w=window.open('','_blank');if(!w)return;w.document.write(`<html><head><title>${$('v9-ot-titulo').textContent}</title><style>body{font-family:Arial,sans-serif;padding:30px;color:#1b2f4a}h1{border-bottom:2px solid #1b7884;padding-bottom:10px}h2{margin-top:28px;font-size:18px}.trabajo{padding:8px 0;border-bottom:1px solid #ddd}.avance{margin:15px 0;font-weight:bold}</style></head><body><h1>${$('v9-ot-titulo').textContent}</h1>${$('v9-ot-resumen').innerHTML}<h2>Trabajos autorizados</h2><div class="avance">Avance: ${a.realizados} de ${a.total} (${a.porcentaje}%)</div>${obtenerChecksOT().map(x=>`<div class="trabajo">${x.checked?'☑':'☐'} ${x.parentElement.innerText}</div>`).join('')}<h2>Refacciones / caja</h2><p>Las piezas retiradas deberan colocarse en la caja correspondiente al vehiculo.</p><h2>Observaciones</h2><p>${escapar($('v9-ot-observaciones').value||'Sin observaciones.')}</p></body></html>`);w.document.close();w.focus();w.print();});
...
// Usa los contratos ya presentes en la base recuperada. Para campos V11 nuevos,
// envía únicamente propiedades que la respuesta de herramientas_listar demuestra
// que existen, evitando inventar nombres de columnas no confirmados.
// ============================================================================
...
$('v9-guardar-herr')?.addEventListener('click',async()=>{
  ...
  // Solo se incluyen campos cuya clave ha sido observada en la fila cargada.
  // Para alta, el contrato recuperado solo confirma codigo/nombre/observaciones.
  if(actual){
    ...
    const codigo=$('v9-herr-codigo')?.value.trim();
    if(codigo)registro.codigo=codigo;
  ...
  cerrarModal('modal-v9-herr');await cargarHerramientas();dashboard();
});
...
function abrirDevolucion(id){
  if(!$('modal-v11-devolucion')||!$('v11-devolucion-id'))return;
...
async function abrirHistorialHerramienta(id){
  ...
  // El v9.js recuperado confirma préstamo/devolución vía RPC, pero no confirma
  // el nombre de la tabla/RPC de historial V11. Se evita inventar un contrato.
  tbody.innerHTML='<tr><td colspan="7" class="vacio-tabla">El frontend V11.1 está listo para mostrar el historial, pero el contrato de consulta del historial no está definido en el v9.js recuperado.</td></tr>';
}
