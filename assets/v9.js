...
(() => {
    // =========================================================================
    // 1. HELPERS Y CONSTANTES GLOBALES
    // =========================================================================
    const $ = id => document.getElementById(id);
    let otId = null;

    function admin() {
        return estado?.perfil?.rol === 'administrador';
    }

    function staff() {
        return ['administrador', 'recepcion'].includes(estado?.perfil?.rol);
    }
    ...
    function aplicarPermisos() {
        ...
        document.querySelectorAll('[data-admin-only]').forEach(x => {
            x.style.display = r === 'administrador' ? '' : 'none';
        });

        if (r === 'tecnico') {
            document.querySelectorAll('.nav-item').forEach(x => {
                if (!['inicio', 'ordenes', 'herramientas'].includes(x.dataset.modulo)) {
    ...
    async function dashboard() {
        ...
        $('dash-ot-abiertas').textContent = data.ot_abiertas || 0;
        $('dash-trabajos-pendientes').textContent = data.trabajos_pendientes || 0;
        $('dash-ot-terminadas').textContent = data.ot_terminadas_hoy || 0;
        $('dash-ingresos-hoy').textContent = '$' + money(data.ingresos_hoy || 0);

        $('dash-avance-ordenes').innerHTML = (data.ordenes || []).map(o => `
            <div>
                <strong>${o.folio}</strong> · ${o.placa} · ${o.realizados}/${o.total}
                <div class="v9-progreso">
                    <span style="width:${o.total ? (o.realizados / o.total) * 100 : 0}%"></span>
                </div>
            </div>
        `).join('') || '<small>Sin órdenes abiertas.</small>';

        $('dash-herramientas').textContent = `Disponibles ${data.herramientas_disponibles || 0} · Prestadas ${data.herramientas_prestadas || 0} · Fuera de servicio ${data.herramientas_fuera || 0}`;

        $('dash-pagos').innerHTML = (data.ultimos_pagos || []).map(p => `
            <div>${p.folio} · $${money(p.importe)} · ${p.metodo}</div>
        `).join('') || '<small>Sin pagos hoy.</small>';
    }

    // =========================================================================
    // 4. COTIZACIONES Y DESCUENTOS
    // =========================================================================
    ...
    function calcDesc() {
        ...
        let d = 0;
        if (tipo === 'porcentaje') {
            d = Math.min(sub, (sub * Math.min(v, 100)) / 100);
        } else if (tipo === 'monto') {
            d = Math.min(sub, v);
        }

        estado.conceptosEnEdicion.forEach(c => c.descuento = 0);
        if (estado.conceptosEnEdicion[0]) {
            estado.conceptosEnEdicion[0].descuento = d;
        }

        $('v9-desc-texto').textContent = tipo === 'porcentaje'
            ? `${v}%`
            : tipo === 'monto'
                ? `$${money(v)}`
                : 'Sin descuento';
    ...
    document.querySelectorAll('input[name="v9-desc-tipo"]').forEach(r => {
        r.addEventListener('change', () => {
            $('v9-desc-valor').disabled = r.checked && r.value === 'ninguno';
            calcDesc();
        });
    });
    ...
    if (renderOriginal) {
        window.renderConceptos = function () {
            renderOriginal();
            document.querySelectorAll('#cuerpo-conceptos [data-campo="descuento"]').forEach(i => {
                i.closest('td').style.display = 'none';
            });
            bloquearCot();
        };
    }
    ...
    function bloquearCot() {
        ...
        $('v9-adicionales').style.display = aut ? 'block' : 'none';
        $('btn-v9-ot').style.display = aut ? 'inline-block' : 'none';

        if (aut && !admin()) {
            document.querySelectorAll('#cuerpo-conceptos [data-campo]').forEach(i => {
                i.disabled = true;
                i.classList.add('v9-lock');
            });
            document.querySelectorAll('#cuerpo-conceptos [data-quitar]').forEach(b => b.style.display = 'none');
            $('btn-agregar-concepto').style.display = 'none';
            $('btn-cat-agregar').style.display = 'none';
            document.querySelectorAll('.v9-descuento input').forEach(i => i.disabled = true);

            const e = $('cotizacion-estado-comercial');
            [...e.options].forEach(o => o.disabled = !['autorizada', 'cerrada'].includes(o.value));
        } else if (aut && admin()) {
            const e = $('cotizacion-estado-comercial');
            [...e.options].forEach(o => o.disabled = !['autorizada', 'cerrada', 'cancelada'].includes(o.value));
        }
    }
    ...
    async function adicionales() {
        const id = $('cotizacion-id')?.value;
        if (!id) return;

        const { data } = await sb.from('cotizacion_adicionales')
            .select('*')
            .eq('cotizacion_id', id)
            .order('created_at');

        $('v9-lista-adicionales').innerHTML = (data || []).map(a => `
            <div class="v9-adicional">
                <b>${a.descripcion}</b> · ${a.cantidad} × $${money(a.precio_unitario)} 
                <span class="badge ${a.estado === 'autorizado' ? 'verde' : a.estado === 'no_autorizado' ? 'rojo' : 'naranja'}">
                    ${a.estado}
                </span>
                ${a.observacion ? `<div>${a.observacion}</div>` : ''}
                ${a.estado === 'pendiente' && staff() ? `
                    <button class="btn pequeno" data-a="${a.id}">Autorizar</button> 
                    <button class="btn secundario pequeno" data-n="${a.id}">No autorizar</button>
                ` : ''}
            </div>
        `).join('') || '<small>Sin adicionales.</small>';

        document.querySelectorAll('[data-a]').forEach(b => {
            b.onclick = async () => {
                await sb.rpc('resolver_adicional', { p_adicional_id: b.dataset.a, p_estado: 'autorizado' });
                adicionales();
            };
        });
        ...
        document.querySelectorAll('[data-n]').forEach(b => {
            b.onclick = async () => {
                await sb.rpc('resolver_adicional', { p_adicional_id: b.dataset.n, p_estado: 'no_autorizado' });
                adicionales();
            };
        });
    }
    ...
    $('v9-guardar-adicional')?.addEventListener('click', async () => {
        const { error } = await sb.from('cotizacion_adicionales').insert({
            cotizacion_id: $('cotizacion-id').value,
            descripcion: $('v9-ad-desc').value.trim(),
            cantidad: Number($('v9-ad-cant').value || 1),
            precio_unitario: Number($('v9-ad-precio').value || 0),
            observacion: $('v9-ad-obs').value.trim() || null,
            created_by: estado.usuario.id
        });

        if (!error) {
            cerrarModal('modal-v9-adicional');
            adicionales();
        }
    });
    ...
    async function cargarOT() {
        let q = sb.rpc('ordenes_trabajo_listar');
        const { data } = await q;
        let l = data || [];

        if (estado.perfil?.rol === 'tecnico') {
            l = l.filter(x => x.tecnico_id === estado.usuario.id);
        }
    ...
    function renderOT() {
        const txt = ($('buscar-orden').value || '').toLowerCase();
        const f = $('filtro-orden').value;
        let l = window.v9OT || [];

        if (txt) {
            l = l.filter(x => [x.folio, x.cotizacion_folio, x.placa].some(v => (v || '').toLowerCase().includes(txt)));
        }
        if (f) {
            l = l.filter(x => x.estado === f);
        }

        $('tabla-ordenes').innerHTML = l.map(x => `
            <tr>
                <td>${x.folio}</td>
                <td>${x.cotizacion_folio}</td>
                <td>${x.placa} · ${x.vehiculo}</td>
                <td>${x.tecnico || 'Sin asignar'}</td>
                <td>${x.realizados}/${x.total}</td>
                <td>${x.estado}</td>
                <td><button class="btn secundario pequeno" data-ot="${x.id}">Abrir</button></td>
            </tr>
        `).join('');

        document.querySelectorAll('[data-ot]').forEach(b => {
            b.onclick = () => abrirOT(b.dataset.ot);
        });
    }

    $('buscar-orden')?.addEventListener('input', renderOT);
    $('filtro-orden')?.addEventListener('change', renderOT);

    $('btn-v9-ot')?.addEventListener('click', async () => {
        const { data, error } = await sb.rpc('generar_orden_trabajo', { p_cotizacion_id: $('cotizacion-id').value });
        if (!error) abrirOT(data);
    });
    ...
    $('v9-guardar-ot')?.addEventListener('click', async () => {
        ...
        if (error) {
            ...
            alert(error.message || 'No fue posible guardar el avance.');
            return;
        }
    ...
    $('v9-imprimir-ot')?.addEventListener('click', () => {
        ...
        w.document.write(`
            <html>
                <head>
                    <title>${$('v9-ot-titulo').textContent}</title>
                    <style>
                        body { font-family: Arial, sans-serif; padding: 30px; color: #1b2f4a; }
                        h1 { border-bottom: 2px solid #1b7884; padding-bottom: 10px; }
                        h2 { margin-top: 28px; font-size: 18px; }
                        .trabajo { padding: 8px 0; border-bottom: 1px solid #ddd; }
                        .avance { margin: 15px 0; font-weight: bold; }
                    </style>
                </head>
                <body>
                    <h1>${$('v9-ot-titulo').textContent}</h1>
                    ${$('v9-ot-resumen').innerHTML}
                    <h2>Trabajos autorizados</h2>
                    <div class="avance">Avance: ${avance.realizados} de ${avance.total} (${avance.porcentaje}%)</div>
                    ${obtenerChecksOT().map(x => `<div class="trabajo">${x.checked ? '☑' : '☐'} ${x.parentElement.innerText}</div>`).join('')}
                    <h2>Refacciones / caja</h2>
                    <p>Las piezas retiradas deberan colocarse en la caja correspondiente al vehiculo.</p>
                    <h2>Observaciones</h2>
                    <p>${$('v9-ot-observaciones').value || 'Sin observaciones.'}</p>
                </body>
            </html>
        `);
    ...
    async function cargarCarga() {
        ...
        const mes = $('carga-mes').value || new Date().toISOString().slice(0, 7);
        $('carga-mes').value = mes;

        const { data } = await sb.rpc('carga_trabajo_operativa', { p_mes: mes });

        $('carga-tecnicos').innerHTML = (data || []).map(x => `
            <div class="panel">
                <h3>${x.tecnico}</h3>
                <p>Órdenes abiertas: <b>${x.ordenes_abiertas}</b> · Trabajos pendientes: <b>${x.trabajos_pendientes}</b> · Órdenes atendidas en el mes: <b>${x.ordenes_mes}</b></p>
            </div>
        `).join('') || '<div class="panel">Sin datos.</div>';
    }
    ...
    async function cargarIngresos() {
        const f = $('ingresos-fecha').value || hoy();
        $('ingresos-fecha').value = f;

        const { data } = await sb.rpc('ingresos_por_dia', { p_fecha: f });
        let l = data || [];
        let m = $('ingresos-metodo').value;

        if (m) {
            l = l.filter(x => x.metodo === m);
        }

        const sum = metodo => l
            .filter(x => x.estado === 'valido' && (!metodo || x.metodo === metodo))
            .reduce((s, x) => s + Number(x.importe), 0);

        $('ing-total').textContent = '$' + money(sum());
        $('ing-efectivo').textContent = '$' + money(sum('efectivo'));
        $('ing-transferencia').textContent = '$' + money(sum('transferencia'));
        $('ing-tarjeta').textContent = '$' + money(sum('tarjeta'));

        $('tabla-ingresos').innerHTML = l.map(x => `
            <tr>
                <td>${new Date(x.fecha_hora).toLocaleTimeString('es-MX')}</td>
                <td>${x.folio}</td>
                <td>${x.cliente} · ${x.placa}</td>
                <td>${x.metodo}</td>
                <td>${x.referencia || '—'}</td>
    ...
    $('ingresos-fecha')?.addEventListener('change', cargarIngresos);
    $('ingresos-metodo')?.addEventListener('change', cargarIngresos);
...
async function cargarHerramientas() {
    ...
    try {
        ...
        const { data, error } =
            await sb.rpc('herramientas_listar');

        if (error) {
            console.error(error);
            alert(
                error.message ||
                'No fue posible cargar herramientas.'
            );
            return;
        }
        ...
        alert(
            'Error inesperado al cargar herramientas.'
        );
...
function renderHerr() {
    ...
    const texto =
        ($('buscar-herramienta')?.value || '')
        .toLowerCase()
        .trim();

    const filtro =
        $('filtro-herramienta')?.value || '';
    ...
    document
        .querySelectorAll('[data-devolver]')
        .forEach(b => {
            ...
            b.onclick = async () => {
                ...
                const confirmar =
                    confirm(
                        '¿Deseas devolver esta herramienta?'
                    );
                ...
                if (error) {
                    ...
                    alert(
                        error.message ||
                        'No fue posible devolver la herramienta.'
                    );
...
async function prestamo(id) {
    ...
    $('v9-prestamo-id').value = id;

    const [
        { data: tecnicos, error: errTec },
        { data: ordenes, error: errOt }
    ] = await Promise.all([
...
?.addEventListener(
    ...
    window.cargarBitacora = async function () {
        ...
        const { data } = await sb.rpc('bitacora_amigable');

        $('tabla-bitacora').innerHTML = (data || []).map(x => `
            <tr>
                <td>${new Date(x.fecha).toLocaleString('es-MX')}</td>
                <td>${x.usuario_nombre || 'Sistema'}</td>
                <td>${x.resumen}</td>
                <td>
                    <button class="btn secundario pequeno" data-det='${encodeURIComponent(JSON.stringify(x.detalle || {}))}'>Ver detalle</button>
                </td>
            </tr>
        `).join('');
        ...
        document.querySelectorAll('[data-det]').forEach(b => {
            b.onclick = () => {
                const detalles = Object.entries(JSON.parse(decodeURIComponent(b.dataset.det)))
