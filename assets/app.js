...
async function iniciarSesionExitosa(session) {
  ...
  // navegador aún tenía el token guardado), porque nunca se revisaba
  // este campo al iniciar sesión.
  if (!perfil || perfil.activo === false) {
    await sb.auth.signOut();
    el("pantalla-login").style.display = "flex";
    el("app-shell").classList.remove("activo");
...
el("cliente-buscar")?.addEventListener("input", () => {
  ...
  _debCliente = setTimeout(async () => {
    ...
    cont.innerHTML = data.map(c => `<div class="sugerencia-item" data-cli="${c.id}" data-nombre="${escHtml(c.nombre_completo)}">${escHtml(c.nombre_completo)}<small>${escHtml(c.telefono)||"sin teléfono"} · ${escHtml(c.correo)||"sin correo"} · ${c.autos} auto(s)</small></div>`).join("");
...
el("ncli-telefono")?.addEventListener("input", () => {
  ...
  _debTel = setTimeout(async () => {
    ...
    if (data && data.length) {
      alerta.innerHTML = `<span style="color:var(--orange);">⚠ Ya existe <strong>${escHtml(data[0].nombre_completo)}</strong> con este teléfono. <a href="#" id="usar-existente">Usarlo</a></span>`;
      el("usar-existente").addEventListener("click", (e) => { e.preventDefault(); seleccion.clienteId = data[0].id; document.querySelector('input[name="modo-cliente"][value="existente"]').checked = true; el("cliente-modo-nuevo").style.display = "none"; el("cliente-modo-existente").style.display = "block"; el("cliente-elegido").style.display = "flex"; el("cliente-elegido").innerHTML = `✓ Cliente: <strong>${escHtml(data[0].nombre_completo)}</strong>`; refrescarAutosDelCliente(); });
...
el("nveh-vin")?.addEventListener("input", () => {
  ...
  _debVin = setTimeout(async () => {
    ...
    if (data && data.length) {
      alerta.innerHTML = `<span style="color:var(--orange);">⚠ Este VIN ya existe (${escHtml(data[0].marca)} ${escHtml(data[0].modelo)}, placa ${escHtml(data[0].placa)}). <a href="#" id="usar-veh">Usar ese auto</a></span>`;
...
async function abrirCotizacion(id) {
  ...
  if (id) {
    ...
    if (el("v9-lista-adicionales")) el("v9-lista-adicionales").innerHTML = "";
    if (typeof window.bloquearCot === "function") window.bloquearCot();
  }
  renderConceptos();
