...
async function adicionales(){
  ...
  if(!id){ if($('v9-lista-adicionales')) $('v9-lista-adicionales').innerHTML=''; return; }
  ...
  window._v9AdicionalesCache = data || [];
  $('v9-lista-adicionales').innerHTML=(data||[]).map(a=>`
    <div class="v9-adicional">
      <b>${escapar(a.descripcion)}</b> · ${a.cantidad} × $${money(a.precio_unitario)}
      <span class="badge ${a.estado==='autorizado'?'verde':a.estado==='no_autorizado'?'rojo':'naranja'}">${escapar(a.estado)}</span>
      ${a.observacion?`<div>${escapar(a.observacion)}</div>`:''}
      ${a.estado==='pendiente'&&staff()?`
        <div style="display:flex;gap:6px;align-items:center;margin-top:6px;flex-wrap:wrap;">
          <select data-tipo-adicional="${a.id}" style="padding:5px 8px;border:1px solid var(--border);border-radius:6px;font-size:12.5px;">
            <option value="refaccion_libre">Refacción</option>
  ...
  document.querySelectorAll('[data-a]').forEach(b=>b.onclick=async()=>{
    const adicionalId = b.dataset.a;
    const adicional = (window._v9AdicionalesCache||[]).find(x=>String(x.id)===String(adicionalId));
    const selTipo = document.querySelector(`[data-tipo-adicional="${adicionalId}"]`);
    const tipoElegido = selTipo ? selTipo.value : 'refaccion_libre';

    const {error} = await sb.rpc('resolver_adicional',{p_adicional_id:adicionalId,p_estado:'autorizado'});
...
function configurarSeleccionTodosOT(){const t=$('v9-seleccionar-todos');if(!t)return;t.onchange=()=>{if(window.v9OTTerminada)return;const val=t.checked;obtenerChecksOT().forEach(chk=>{chk.checked=val;});actualizarAvanceOT();};}
